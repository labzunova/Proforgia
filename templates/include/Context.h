...
namespace templates { //все классы, рабтающие с шаблонами будут относиться к данному пространству имен
    ...
    public:
        Context() = default;
        explicit Context(const std::string &json); //получение данных формата json в качестве просто строки
        std::string fromPtreeToStr() const; //созданеи из boost::property_tree::ptree(возможно) просто строку json
        std::map<std::string, std::string> fromPrteeToMap(); //для создания мапы
        template<class T>
        void put(const std::string &name, const T &value); //методы положить и взять в root данные по ключу(удобно для заполнения данных контекста, чтоб потом отдать его на рендеринг)
        template<class T>
        T get(const std::string &name) const;
    private:
       std::map<std::string, std::string> root; //возможна реализация с помощью типа  boost::property_tree::ptree -удобная древовидна структура отображения данных(библиотека имеет встроенный парсер)

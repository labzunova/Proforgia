...
public:
	virtual bool add_file_to_storage( const std::string& filename, const std::string& location, ErrorCodes &error ) const = 0;
	virtual std::string get_file( const std::string& filename, ErrorCodes &error ) const = 0; // return path to file on server
